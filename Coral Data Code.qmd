...
m_random <- lmer(total_size ~ mean_recruits + Fleshy_MA + Treatment + 
                   (1|Year), data = all_data)
summary(m_random)
...
resid_vals <- resid(m_fixed)        # raw residuals
fitted_vals <- fitted(m_fixed)      # fitted values



hist(resid_vals, breaks = 15, main = "Histogram of residuals", xlab = "Residuals")
plot(density(resid_vals), main = "Density of residuals")
...
# Random intercepts for Block and Year
m_block <- lmer(total_size ~ mean_recruits + Fleshy_MA + Treatment + 
                  (1|Block) + (1|Year),
                data = all_data)
...
# Make sure Treatment is a factor with Con as the baseline
all_data$Treatment <- factor(all_data$Treatment, 
                             levels = c("Con", "1x1", "2x2", "TopOnly"))

# Global model (everything you want dredge to consider)
m_global <- lmer(total_size ~ mean_recruits + Fleshy_MA + CCA_calcareous + 
                   `Lobo/Peys` + SPAT + TAS +
                   `Grazerdetritivore_10-20cm` + `Grazerdetritivore_20-30cm` +
                   `Scrapper_10-20cm` + `Scrapper_20-30cm` + `Scrapper_>30cm` +
                   Treatment + Year +           # include Year if you want it dredged
                   (1|Block) + (1|Year),        # random effects (fixed in place)
                 data = all_data, REML = FALSE)

# 🔹 Tell MuMIn to fail if missing data would cause problems
options(na.action = "na.fail")

# Now dredge works as intended
model_set <- dredge(m_global, rank = "AICc")
head(model_set, 10)
...
# --- Stable factor order for Treatment (Control as baseline) ---
all_data$Treatment <- factor(all_data$Treatment, levels = c("Con","1x1","2x2","TopOnly"))

# Also prepare a factor version of Year for the "Year as fixed categorical" branch
all_data$Year_fac <- factor(all_data$Year)

# --- Pools of candidate fixed effects (USE RAW COLUMN NAMES HERE; NO BACKTICKS) ---
base_terms   <- c("mean_recruits", "Treatment")
benthic_pool <- c("Fleshy_MA","CCA_calcareous","Lobo/Peys","SPAT","TAS")
fish_pool    <- c("Grazerdetritivore_10-20cm","Grazerdetritivore_20-30cm",
                  "Scrapper_10-20cm","Scrapper_20-30cm","Scrapper_>30cm")

# --- Quick sanity check for typos/missing columns ---
missing_cols <- setdiff(
  c(base_terms, benthic_pool, fish_pool, "Block","Year","Year_fac","total_size"),
  names(all_data)
)
if (length(missing_cols)) {
  stop(sprintf("These columns are missing from all_data: %s",
               paste(missing_cols, collapse = ", ")))
}

# --- Helper: wrap only names that need backticks for use INSIDE formulas ---
wrap_special <- function(x) ifelse(grepl("[^[:alnum:]_]", x), paste0("`", x, "`"), x)

# --- Helper: build formula safely (fixed vars + year term + random structure) ---
build_formula <- function(response, fixed_vars, year_term, re_term) {
  rhs_fixed <- if (length(fixed_vars)) paste(wrap_special(fixed_vars), collapse = " + ") else "1"
  rhs <- paste(c(rhs_fixed, wrap_special(year_term), re_term), collapse = " + ")
  as.formula(paste(response, "~", rhs))
}

# --- Runs to search (keeps sample sizes reasonable) ---
runs <- list(
  list(name = "benthic", fixed_pool = c(base_terms, benthic_pool)),
  list(name = "fish",    fixed_pool = c(base_terms, fish_pool)),
  list(name = "both",    fixed_pool = c(base_terms, benthic_pool, fish_pool))
)

# Year representations to consider
year_modes <- list(
  list(name = "Year_numeric", term = "Year"),
  list(name = "Year_factor",  term = "Year_fac")
)

# Random-effect structures to compare (dredge does not vary these)
rand_structs <- list(
  list(name = "RB_RY",  re = "(1|Block) + (1|Year)"),
  list(name = "RB_only",re = "(1|Block)")
  # Add others you used manually, e.g. list(name="RY_only", re="(1|Year)"),
  # or lm-only case by fitting lm() separately if needed
)

# MuMIn requires this for exhaustive subset search
options(na.action = "na.fail")

# --- Function: fit one configuration and dredge its fixed-effect subsets ---
fit_and_dredge <- function(fixed_pool, year_term, re_term, run_label) {
  # Build minimal dataset with complete cases ONLY for the variables needed in this run
  vars_needed <- unique(c("total_size", fixed_pool, "Treatment", "Block", "Year", year_term))
  df <- all_data %>%
    dplyr::select(all_of(vars_needed)) %>%
    tidyr::drop_na() %>%
    mutate(Treatment = factor(Treatment, levels = c("Con","1x1","2x2","TopOnly")))

  if (nrow(df) < 10) return(NULL)  # too few rows to fit sensibly

  # Fit global mixed model for this configuration
  fml <- build_formula("total_size", fixed_pool, year_term, re_term)
  m_global <- lmer(fml, data = df, REML = FALSE)

  # All-subsets on fixed effects
  ms <- dredge(m_global, rank = "AICc")
  ms$run        <- run_label
  ms$N          <- nrow(df)
  ms$year_mode  <- year_term
  ms$random_str <- re_term

  list(modelset = ms, global = m_global, data = df)
}
...
for (r in runs) {
  for (ym in year_modes) {
    for (rs in rand_structs) {
      lbl <- paste(r$name, ym$name, rs$name, sep = "_")
      out <- fit_and_dredge(r$fixed_pool, ym$term, rs$re, lbl)
      if (!is.null(out)) { results[[k]] <- out; k <- k + 1 }
    }
  }
}

# --- Collect and display the best models ---
all_tables <- dplyr::bind_rows(lapply(results, `[[`, "modelset"))

best_per_config <- all_tables %>%
  group_by(run, year_mode, random_str, N) %>%
  slice_min(order_by = AICc, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  arrange(AICc)

cat("\n--- Best model per configuration (sorted by AICc) ---\n")
print(best_per_config)

overall_top <- best_per_config %>% slice_min(order_by = AICc, n = 5, with_ties = TRUE)
cat("\n--- Overall top models across all configurations ---\n")
print(overall_top)

# --- Extract and summarize the single best model we found ---
if (nrow(overall_top) > 0) {
  top <- overall_top[1, ]
  # Find the matching result entry
  idx <- which(sapply(results, function(x) {
    any(x$modelset$AICc == top$AICc & x$modelset$run == top$run)
  }))[1]

  top_model <- get.models(results[[idx]]$modelset, subset = 1)[[1]]
  cat("\n\n--- Summary of overall best model ---\n")
  print(summary(top_model))
  cat("\nN used:", results[[idx]]$modelset$N[1],
      "\nYear mode:", results[[idx]]$modelset$year_mode[1],
      "\nRandom:", results[[idx]]$modelset$random_str[1], "\n")
}
